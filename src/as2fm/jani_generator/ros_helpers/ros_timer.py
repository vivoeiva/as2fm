...
def make_global_timer_scxml(
    timers: List[RosTimer], max_time_ns: int
) -> Tuple[Optional[AscxmlRootROS], Optional[ModelTimeStep]]:
    """
    Create a global timer SCXML automaton from a list of ROS timers.

    :param timers: The list of ROS timers.
    :return: The global timer SCXML and the model time step.
    """
    if len(timers) == 0:
        return None, None
    global_timer_period, global_timer_period_unit = get_gcd_of_timer_periods(timers)
    model_time_step = ModelTimeStep(global_timer_period, global_timer_period_unit)
    timers_map = {
    ...
    return scxml_root, model_time_step
